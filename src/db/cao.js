...
const femalesArr = data.filter((pObj) => pObj.gender === 'Female');
console.log('femalesArr ===', femalesArr);

const namesArr = femalesArr.map((pObj) => {
  const { first_name: name, last_name: surname } = pObj;
  return `${name} ${surname}`;
});

const rez = data
  .filter((pObj) => pObj.gender === 'Female')
  .map(({ first_name: name, last_name: surname }) => `${name} ${surname}`);
